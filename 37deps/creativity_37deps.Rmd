...
df <-
  ...
  ungroup() %>% 
  mutate_at(vars(contains("tpi")), ~ scale(.) %>% as.vector)
```
...
lm1 <-
  df %>% 
  lm_robust(z_fluency ~ tpi_average_demos + female + age + father_college + mother_college + ses, se_type = "stata", clusters = classid, data = .)

lm2 <-
  df %>% 
  lm_robust(z_uniqueness ~ tpi_average_demos + female + age + father_college + mother_college + ses, se_type = "stata", clusters = classid, data = .)

lm3 <-
  df %>% 
  lm_robust(z_flexibility ~ tpi_average_demos + female + age + father_college + mother_college + ses, se_type = "stata", clusters = classid, data = .)


screenreg(list(lm1, lm2, lm3), custom.note = "%stars \n All continuous variables are standardized. \n All models control for student background characteristics. \n Standard errors clustered at department level.", omit.coef = "(female)|(college)|(ses)", include.ci = FALSE, stars = c(0.01, 0.05, 0.1), custom.model.names = c("Fluency", "Uniqueness", "Flexibility"))
```
